...
export function initAnm() {
  (window as any).setupGameSelectorForAnm = ($e: HTMLElement) => setupGameSelector(INS_HANDLERS, $e);
  (window as any).generateAnmInsTableHtml = () => generateTablePageHtml(INS_HANDLERS);
  (window as any).generateAnmVarTableHtml = () => generateTablePageHtml(VAR_HANDLERS);
...
export type AnmInsData = {
  sig: string,
  args: string[],
} & CommonData;

export type AnmVarData = {
  type: VarType,
  mut: boolean,
} & CommonData;
...
type TableHandlers<Data> = {
  ...
  generateTableRowHtml(game: Game, data: Data, opcode: number): string,

  /**
   * Markdown message to generate in place of a table when there's no rows.
   * `null` when the table is not expected to be empty.
   */
  emptyTableMessage(c: Context): string | null,
}
...
export const INS_HANDLERS: TableHandlers<AnmInsData> = makeTableHandlers({
  tablePage: 'anm/ins',
...
export const VAR_HANDLERS: TableHandlers<AnmVarData> = makeTableHandlers({
  tablePage: 'anm/var',
  ...
  emptyTableMessage: (c: Context) => (c.g === '06') ? '**EoSD ANM has no variables, _nerrrrd._**' : null,
});
...
const ARGTYPES_HTML: StrMap<string, string> = {
...
function initAnmVarNames() {
  ...
  for (const version of SUPPORTED_ANM_VERSIONS) {
    ...
    ANM_VAR_NAMES.registerPrefix(version, (suffix, ctx) => {
      ...
      const entry = VAR_HANDLERS.tableByOpcode.get(game)![opcode];
...
function registerRefNamesForTable<D>(tableHandlers: TableHandlers<D>) {
  const {mainPrefix, reverseTable, gameRangeTable} = tableHandlers;
  globalRefNames.registerPrefix(mainPrefix, (id, ctx) => {
    ...
    function preferredVersion() {
      ...
      return GAME_ANM_VERSIONS[avail.maxGame];
    }

    const version = preferredVersion();
    if (!version) return null; // no game has it
...
function generateTablePageHtml<D extends CommonData>(tableHandlers: TableHandlers<D>) {
  const {getGroups, dummyDataWhenNoRef, mainPrefix, generateTableRowHtml, emptyTableMessage} = tableHandlers;
  ...
  for (const group of groups) {
    ...
    for (let opcode=group.min; opcode<=group.max; ++opcode) {
      ...
      total += 1;
      table += generateTableRowHtml(game, data, opcode);
    }
  ...
  if (total === 0) {
    base += '<br>' + (emptyTableMessage(currentQuery) || "No data...?");
  } else {
...
function generateAnmInsSiggy(ins: AnmInsData, {nameKey}: {nameKey: string}) {
...
function generateAnmVarHeader(avar: AnmVarData, {nameKey}: {nameKey: string}) {
  const name = globalNames.getHtml(nameKey);
  return /* html */`<span class="var-header" data-wip="${avar.wip}">${name}</span>`;
}

function generateInsTableRowHtml(game: Game, ins: AnmInsData, opcode: number) {
  const nameKey = INS_HANDLERS.getNameKey(game, opcode);
...
function generateVarTableRowHtml(game: Game, ins: AnmVarData, opcode: number) {
  const nameKey = VAR_HANDLERS.getNameKey(game, opcode);
  let [desc] = handleTipHide(ins.desc, false);
  desc = postprocessAnmDesc(desc, false);
...
function generateAnmInsParameters(ins: AnmInsData) {
...
function getUrlByRef<D>(ref: string, context: Context, tableHandlers: TableHandlers<D>) {
  ...
  if (queryPageEquals(context, {s: tablePage})) {
    ...
    query = queryFilterCommonProps(context);
  }
